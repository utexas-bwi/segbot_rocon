...
class SegbotManager(RobotManager):
    ...
    def prepare_rocon_launch_text(self, robots):
        ...
        for name in robots:
            ...
            launch_text += '    <arg name="robot_concert_whitelist" value="BWI Concert;Gazebo Concert;Concert Tutorial"/>\n'
            launch_text += '    <arg name="robot_rapp_whitelist" value="rocon_apps, segbot_rapps, segbot_gazebo_concert"/>\n'
            launch_text += '  </launch>\n'
            port = port + 1
        launch_text += '</concert>\n'
        return launch_text
